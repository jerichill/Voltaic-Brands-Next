import React from 'react'
import Link from 'next/link'
import Head from 'next/head'

import SolidButton from '../components/solid-button'

const PrivacyPolicy = (props) => {
  return (
    <>
      <div className="privacy-policy-container">
        <Head>
          <title>
            Privacy Policy - VOLTAIC Brands | Creator of Crypto-Native Digital
            Goods & Merchandise
          </title>
          <meta
            name="description"
            content="VOLTAIC Brands creates innovative digital goods, collectibles, and merchandise for the next generation of crypto-native consumers."
          />
          <meta
            property="og:title"
            content="Privacy Policy - VOLTAIC Brands | Creator of Crypto-Native Digital Goods &amp; Merchandise"
          />
          <meta
            property="og:description"
            content="VOLTAIC Brands creates innovative digital goods, collectibles, and merchandise for the next generation of crypto-native consumers."
          />
          <meta
            property="og:image"
            content="https://aheioqhobo.cloudimg.io/v7/_playground-bucket-v2.teleporthq.io_/1db061e4-ab2f-4c92-8ac0-93b75c2334a5/bc7bb89d-dc00-4857-8f14-2762d9f3ffe0?org_if_sml=1"
          />
        </Head>
        <header className="privacy-policy-top-container">
          <nav data-role="Header" className="privacy-policy-navbar">
            <Link href="/home">
              <a className="privacy-policy-link">
                <span className="privacy-policy-text">
                  VOLTAIC
                  <span
                    dangerouslySetInnerHTML={{
                      __html: ' ',
                    }}
                  />
                </span>
                <span className="privacy-policy-text001">Brands</span>
              </a>
            </Link>
            <span className="privacy-policy-text002">TM</span>
            <div className="privacy-policy-right-side">
              <div className="privacy-policy-links-container">
                <span className="privacy-policy-text003 menu">Home</span>
                <span className="privacy-policy-text004 menu">About</span>
                <span className="privacy-policy-text005 menu">
                  Tour Packages
                </span>
                <span className="privacy-policy-text006 menu">Contact</span>
              </div>
              <a href="#main-section">
                <SolidButton button="Explore places"></SolidButton>
              </a>
            </div>
            <div data-type="BurgerMenu" className="privacy-policy-burger-menu">
              <svg
                viewBox="0 0 1024 1024"
                className="privacy-policy-burger-menu"
              >
                <path d="M810.667 725.333h-597.333c-47.061 0-85.333 38.272-85.333 85.333s38.272 85.333 85.333 85.333h597.333c47.061 0 85.333-38.272 85.333-85.333s-38.272-85.333-85.333-85.333z"></path>
                <path d="M810.667 426.667h-597.333c-47.061 0-85.333 38.272-85.333 85.333s38.272 85.333 85.333 85.333h597.333c47.061 0 85.333-38.272 85.333-85.333s-38.272-85.333-85.333-85.333z"></path>
                <path d="M810.667 128h-597.333c-47.061 0-85.333 38.272-85.333 85.333s38.272 85.333 85.333 85.333h597.333c47.061 0 85.333-38.272 85.333-85.333s-38.272-85.333-85.333-85.333z"></path>
              </svg>
            </div>
            <div data-type="MobileMenu" className="privacy-policy-mobile-menu">
              <div className="privacy-policy-nav">
                <div className="privacy-policy-top">
                  <h1>Travel</h1>
                  <div
                    data-type="CloseMobileMenu"
                    className="privacy-policy-close-menu"
                  >
                    <svg
                      viewBox="0 0 1024 1024"
                      className="privacy-policy-icon03"
                    >
                      <path d="M810 274l-238 238 238 238-60 60-238-238-238 238-60-60 238-238-238-238 60-60 238 238 238-238z"></path>
                    </svg>
                  </div>
                </div>
                <div className="privacy-policy-right-side1">
                  <div className="privacy-policy-links-container1">
                    <span className="privacy-policy-text007">Home</span>
                    <span className="privacy-policy-text008">About</span>
                    <span className="privacy-policy-text009">
                      Tour Packages
                    </span>
                    <span>Contact</span>
                  </div>
                  <a href="#main-section" className="privacy-policy-link02">
                    <SolidButton button="Explore places"></SolidButton>
                  </a>
                </div>
              </div>
              <div className="privacy-policy-follow-container">
                <span className="privacy-policy-text011">
                  Follow us on
                  <span
                    dangerouslySetInnerHTML={{
                      __html: ' ',
                    }}
                  />
                </span>
                <div className="privacy-policy-icons-container">
                  <a
                    href="https://instagram.com"
                    target="_blank"
                    rel="noreferrer noopener"
                    className="privacy-policy-link03"
                  >
                    <svg
                      viewBox="0 0 877.7142857142857 1024"
                      className="privacy-policy-icon05"
                    >
                      <path d="M585.143 512c0-80.571-65.714-146.286-146.286-146.286s-146.286 65.714-146.286 146.286 65.714 146.286 146.286 146.286 146.286-65.714 146.286-146.286zM664 512c0 124.571-100.571 225.143-225.143 225.143s-225.143-100.571-225.143-225.143 100.571-225.143 225.143-225.143 225.143 100.571 225.143 225.143zM725.714 277.714c0 29.143-23.429 52.571-52.571 52.571s-52.571-23.429-52.571-52.571 23.429-52.571 52.571-52.571 52.571 23.429 52.571 52.571zM438.857 152c-64 0-201.143-5.143-258.857 17.714-20 8-34.857 17.714-50.286 33.143s-25.143 30.286-33.143 50.286c-22.857 57.714-17.714 194.857-17.714 258.857s-5.143 201.143 17.714 258.857c8 20 17.714 34.857 33.143 50.286s30.286 25.143 50.286 33.143c57.714 22.857 194.857 17.714 258.857 17.714s201.143 5.143 258.857-17.714c20-8 34.857-17.714 50.286-33.143s25.143-30.286 33.143-50.286c22.857-57.714 17.714-194.857 17.714-258.857s5.143-201.143-17.714-258.857c-8-20-17.714-34.857-33.143-50.286s-30.286-25.143-50.286-33.143c-57.714-22.857-194.857-17.714-258.857-17.714zM877.714 512c0 60.571 0.571 120.571-2.857 181.143-3.429 70.286-19.429 132.571-70.857 184s-113.714 67.429-184 70.857c-60.571 3.429-120.571 2.857-181.143 2.857s-120.571 0.571-181.143-2.857c-70.286-3.429-132.571-19.429-184-70.857s-67.429-113.714-70.857-184c-3.429-60.571-2.857-120.571-2.857-181.143s-0.571-120.571 2.857-181.143c3.429-70.286 19.429-132.571 70.857-184s113.714-67.429 184-70.857c60.571-3.429 120.571-2.857 181.143-2.857s120.571-0.571 181.143 2.857c70.286 3.429 132.571 19.429 184 70.857s67.429 113.714 70.857 184c3.429 60.571 2.857 120.571 2.857 181.143z"></path>
                    </svg>
                  </a>
                  <a
                    href="https://facebook.com"
                    target="_blank"
                    rel="noreferrer noopener"
                    className="privacy-policy-link04"
                  >
                    <svg
                      viewBox="0 0 602.2582857142856 1024"
                      className="privacy-policy-icon07"
                    >
                      <path d="M548 6.857v150.857h-89.714c-70.286 0-83.429 33.714-83.429 82.286v108h167.429l-22.286 169.143h-145.143v433.714h-174.857v-433.714h-145.714v-169.143h145.714v-124.571c0-144.571 88.571-223.429 217.714-223.429 61.714 0 114.857 4.571 130.286 6.857z"></path>
                    </svg>
                  </a>
                  <a
                    href="https://twitter.com"
                    target="_blank"
                    rel="noreferrer noopener"
                    className="privacy-policy-link05"
                  >
                    <svg
                      viewBox="0 0 950.8571428571428 1024"
                      className="privacy-policy-icon09"
                    >
                      <path d="M925.714 233.143c-25.143 36.571-56.571 69.143-92.571 95.429 0.571 8 0.571 16 0.571 24 0 244-185.714 525.143-525.143 525.143-104.571 0-201.714-30.286-283.429-82.857 14.857 1.714 29.143 2.286 44.571 2.286 86.286 0 165.714-29.143 229.143-78.857-81.143-1.714-149.143-54.857-172.571-128 11.429 1.714 22.857 2.857 34.857 2.857 16.571 0 33.143-2.286 48.571-6.286-84.571-17.143-148-91.429-148-181.143v-2.286c24.571 13.714 53.143 22.286 83.429 23.429-49.714-33.143-82.286-89.714-82.286-153.714 0-34.286 9.143-65.714 25.143-93.143 90.857 112 227.429 185.143 380.571 193.143-2.857-13.714-4.571-28-4.571-42.286 0-101.714 82.286-184.571 184.571-184.571 53.143 0 101.143 22.286 134.857 58.286 41.714-8 81.714-23.429 117.143-44.571-13.714 42.857-42.857 78.857-81.143 101.714 37.143-4 73.143-14.286 106.286-28.571z"></path>
                    </svg>
                  </a>
                </div>
              </div>
            </div>
          </nav>
          <div className="privacy-policy-hero">
            <div className="privacy-policy-content-container">
              <h1 className="privacy-policy-text012">
                <span>Privacy Policy</span>
              </h1>
            </div>
          </div>
        </header>
        <div id="main-section" className="privacy-policy-main">
          <span className="privacy-policy-text014 content">
            <span>COOKIE NOTICE:</span>
            <br></br>
            <span>
              This website uses cookies. IF YOU DO NOT CONSENT TO COOKIES PLEASE
              LEAVE IMMEDIATLY.
            </span>
            <br></br>
            <span></span>
            <br></br>
            <span>
              THIS PRIVACY NOTICE (or “Privacy Policy”) shall apply to all,
              End-Users, and Website Visitors (“Visitors” or “You”) who use the
              portfolio of websites, products, and services provided by
              TurboVirtual (“Us” or “We” or “Our”) located at turbovirtual.com,
              voltaicbrands.com, iamgalactify.com, radpups.com, rareorbs.com,
              etherfelis.com (including related and associated sub-domains). We
              may update this Privacy Notice from time to time in our sole
              discretion.
            </span>
            <br></br>
            <span></span>
            <br></br>
            <span>Introduction</span>
            <br></br>
            <span></span>
            <br></br>
            <span>
              1. This is a notice to inform you of our policy about all
              information that we record about you. It sets out the conditions
              under which we may process any information that we collect from
              you, or that you provide to us. It covers information that could
              identify you (“personal information”) and information that could
              not. In the context of the law and this notice, “process” means
              collect, store, transfer, use or otherwise act on information.
            </span>
            <br></br>
            <span></span>
            <br></br>
            <span>
              2. We regret that if there are one or more points below with which
              you are not happy, your only recourse is to leave our website
              immediately.
            </span>
            <br></br>
            <span></span>
            <br></br>
            <span>
              3. We take seriously the protection of your privacy and
              confidentiality. We understand that all visitors to our website
              are entitled to know that their personal data will not be used for
              any purpose unintended by them, and will not accidentally fall
              into the hands of a third party.
            </span>
            <br></br>
            <span></span>
            <br></br>
            <span>
              4. We undertake to preserve the confidentiality of all information
              you provide to us, and hope that you reciprocate.
            </span>
            <br></br>
            <span></span>
            <br></br>
            <span>
              5. Except as set out below, we do not share, or sell, or disclose
              to a third party, any information collected through our website,
            </span>
            <br></br>
            <span></span>
            <br></br>
            <span>The bases on which we process information about you</span>
            <br></br>
            <span></span>
            <br></br>
            <span>
              The law requires us to determine under which of six defined bases
              we process different categories of your personal information and
              to notify you of the basis for each category.
            </span>
            <br></br>
            <span></span>
            <br></br>
            <span>
              If a basis on which we process your personal information is no
              longer relevant then we shall immediately stop processing your
              data.
            </span>
            <br></br>
            <span></span>
            <br></br>
            <span>
              If the basis changes then if required by law we shall notify you
              of the change and of any new basis under which we have determined
              that we can continue to process your information.
            </span>
            <br></br>
            <span></span>
            <br></br>
            <span>
              6. Information we process because we have a contractual obligation
              with you
            </span>
            <br></br>
            <br></br>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <span>21. Data may be processed outside the European Union</span>
            <span>
              whether the same objective could be achieved through other means
            </span>
            <br></br>
            <span></span>
            <span></span>
            <span></span>
            <span>
              We may update this privacy notice from time to time as necessary.
              The terms that apply to you are those posted here on our website
              on the day you use our website. We advise you to print a copy for
              your records. If you have any questions regarding our privacy
              notice, please contact us.
            </span>
            <span>Disclosure and sharing of your information</span>
            <span>Specific uses of information you provide to us</span>
            <br></br>
            <span></span>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <span></span>
            <br></br>
            <span>
              We continue to process your information on this basis until you
              withdraw your consent or it can be reasonably assumed that your
              consent no longer exists.
            </span>
            <br></br>
            <br></br>
            <br></br>
            <span>
              8. Information we process for the purposes of legitimate interests
            </span>
            <br></br>
            <span>
              Some cookies may last for a defined period of time, such as one
              day or until you close your browser. Others last indefinitely.
            </span>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <span></span>
            <span></span>
            <br></br>
            <span>
              Free speech is a fundamental right, so we must make a judgment as
              to whose right will be obstructed: yours, or that of the person
              who posted the content that offends you.
            </span>
            <span>
              This is information given to us by you in your capacity as an
              affiliate of us or as a business partner.
            </span>
            <span>
              If combined with other information we know about you from previous
              visits, the data possibly could be used to identify you
              personally, even if you are not signed in to our website.
            </span>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <span></span>
            <span>Access to your own information</span>
            <br></br>
            <span></span>
            <span></span>
            <span>
              We process this information on the basis there is a contract
              between us, or that you have requested we use the information
              before we enter into a legal contract.
            </span>
            <span>
              22.1. At any time you may review or update personally identifiable
              information that we hold about you, by signing in to your account
              on our website.
            </span>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <span></span>
            <br></br>
            <span></span>
            <br></br>
            <span></span>
            <br></br>
            <span>Examples include:</span>
            <br></br>
            <span>
              It allows us to recognize visitors that you have referred to us,
              and to credit to you commission due for such referrals. It also
              includes information that allows us to transfer commission to you.
            </span>
            <br></br>
            <span></span>
            <span>
              You may withdraw your consent at any time by instructing us
              support@turbovirtual.com. However, if you do so, you may not be
              able to use our website or our services further.
            </span>
            <span>
              Although we do not disclose your personal information to any third
              party (except as set out in this notice), we sometimes receive
              data that is indirectly made up from your personal information
              from third parties whose services we use.
            </span>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <span>
              6.4. provide you with suggestions and advice on products, services
              and how to obtain the most from using our website
            </span>
            <span></span>
            <br></br>
            <span>
              Whenever information is transferred between us, you can check that
              it is done so using SSL by looking for a closed padlock symbol or
              other trust mark in your browser’s URL bar or toolbar.
            </span>
            <br></br>
            <br></br>
            <span></span>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <span>Other matters</span>
            <br></br>
            <br></br>
            <span></span>
            <span>
              Except where you have consented to our use of your information for
              a specific purpose, we do not use your information in any way that
              would identify you personally. We may aggregate it in a general
              way and use it to provide class information, for example, to
              monitor the performance of a page on our website.
            </span>
            <br></br>
            <span>
              We may also compile statistics showing information obtained from
              this source to assess the level of service we provide, but not in
              a way that could identify you or any other person.
            </span>
            <span>
              They may also use other technology such as cookies or JavaScript
              to personalize the content of, and to measure the performance of
              their adverts.
            </span>
            <br></br>
            <br></br>
            <br></br>
            <span>12. Information relating to your method of payment</span>
            <span>
              When we receive a complaint, we record all the information you
              have given to us. We use that information to resolve your
              complaint.
            </span>
            <span>
              Provided your request is reasonable and there is no legal basis
              for us to retain it, then at our discretion, we may agree to your
              request to delete personal information that you have posted. You
              can make a request by contacting us at support@turbovirtual.com.
            </span>
            <span>
              Wherever possible, we aim to obtain your explicit consent to
              process this information, for example, by asking you to agree to
              our use of cookies. Sometimes you might give your consent
              implicitly, such as when you send us a message by e-mail to which
              you would reasonably expect us to reply.
            </span>
            <span></span>
            <span></span>
            <br></br>
            <span>
              If you wish us to remove personally identifiable information from
              our website, you may contact us at support@turbovirtual.com. This
              may limit the service we can provide to you.
            </span>
            <br></br>
            <br></br>
            <br></br>
            <span>
              10.3. clicking on an icon next to another visitor’s message to
              convey your agreement, disagreement or thanks
            </span>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <span></span>
            <span></span>
            <br></br>
            <span>
              We may process information on the basis there is a legitimate
              interest, either to you or to us, of doing so.
            </span>
            <span>
              We do not specifically use this information except to allow it to
              be displayed or shared.
            </span>
            <span>29. Compliance with the law</span>
            <br></br>
            <br></br>
            <span>
              We use outsourced services in countries outside the European
              Union.
            </span>
            <span>
              We are subject to the law like everyone else. Sometimes, we must
              process your information to comply with a statutory obligation.
            </span>
            <span>We use cookies in the following ways:</span>
            <span></span>
            <br></br>
            <span></span>
            <br></br>
            <span>
              When we receive any request to access, edit or delete personally
              identifiable information we shall first take reasonable steps to
              verify your identity before granting you access or otherwise
              taking any action. This is important to safeguard your
              information.
            </span>
            <span>
              We attempt to moderate user-generated content, but we are not
              always able to do so as soon as that content is published.
            </span>
            <br></br>
            <br></br>
            <span></span>
            <span></span>
            <br></br>
            <span>
              Accordingly, data obtained within the United States of America or
              any other country could be processed outside the European Union.
            </span>
            <span>22. Access to your personal information</span>
            <span></span>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <span>27. How you can complain</span>
            <br></br>
            <span></span>
            <span></span>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <span>
              10. Information provided on the understanding that it will be
              shared with a third party
            </span>
            <span></span>
            <br></br>
            <br></br>
            <span>
              Except as otherwise mentioned in this privacy notice, we keep your
              personal information only for as long as required by us:
            </span>
            <br></br>
            <br></br>
            <span>6.2. sell products to you</span>
            <span></span>
            <br></br>
            <span>
              Once your information enters the public domain, we have no control
              over what any individual third party may do with it. We accept no
              responsibility for their actions at any time.
            </span>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <span>We may use it in order to:</span>
            <br></br>
            <br></br>
            <br></br>
            <span>The information is not used for any other purpose.</span>
            <span></span>
            <span></span>
            <br></br>
            <span></span>
            <br></br>
            <span></span>
            <span></span>
            <span>
              Our privacy notice has been compiled so as to comply with the law
              of every country or legal jurisdiction in which we aim to do
              business. If you think it fails to satisfy the law of your
              jurisdiction, we should like to hear from you.
            </span>
            <br></br>
            <br></br>
            <span>
              Use of information we collect through automated systems when you
              visit our website
            </span>
            <span>
              Through certain actions when otherwise there is no contractual
              relationship between us, such as when you browse our website or
              ask us to provide you more information about our business,
              including our products and services, you provide your consent to
              us to process information that may be personal information.
            </span>
            <br></br>
            <span>16. Cookies</span>
            <br></br>
            <span>
              Our website allows you to post information with a view to that
              information being read, copied, downloaded, or used by other
              people.
            </span>
            <br></br>
            <span>10.2. uploading an image</span>
            <span></span>
            <span>
              We do not have control over these technologies or the data that
              these parties obtain. Accordingly, this privacy notice does not
              cover the information practices of these third parties.
            </span>
            <span>
              When you contact us, whether by telephone, through our website,
              live chat, or by e-mail, we collect the data you have given to us
              in order to reply with the information you need.
            </span>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <span>
              16.4. to record your answers to surveys and questionnaires on our
              site while you complete them 1.5. to record the conversation
              thread during a live chat with our support team
            </span>
            <br></br>
            <span></span>
            <span></span>
            <span>
              We expect any affiliate or partner to agree to reciprocate this
              policy.
            </span>
            <span>25. Use of site by children</span>
            <span></span>
            <span></span>
            <br></br>
            <br></br>
            <span>18. Our use of re-marketing</span>
            <span>
              If you have given us explicit permission to do so, we may from
              time to time pass your name and contact information to selected
              associates whom we consider may provide services or products you
              would find beneficial, including but not limited to our network of
              independent service providers (sub-contractors) and software
              partners.
            </span>
            <span></span>
            <br></br>
            <span></span>
            <span>
              In order to carry out our obligations under that contract, we must
              process the information you give us. Some of this information may
              be personal information.
            </span>
            <span></span>
            <br></br>
            <br></br>
            <span>23. Removal of your information</span>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <span></span>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <span></span>
            <span>
              It is important to consider when creating a non-fungible token you
              are publishing or minting information to the blockchain and it
              cannot be deleted or removed by anyone.
            </span>
            <span></span>
            <span>
              22.2. To obtain a copy of any information that is not provided on
              our website you may send us a request at support@turbovirtual.com
            </span>
            <br></br>
            <span>
              We record your request and our reply in order to increase the
              efficiency of our customer service program. We keep personally
              identifiable information associated with your messages, such as
              your name, email address, and phone number so as to be able to
              track our communications with you to provide a high-quality
              service.
            </span>
            <span></span>
            <br></br>
            <span></span>
            <br></br>
            <span></span>
            <span>
              At the point of payment, you are using a secure service hosted by
              NFTify.network, your associated blockchain wallet provider, and
              related software not controlled by Us. The payment form may be
              branded to look like Us, but it is not controlled or hosted by us.
            </span>
            <span></span>
            <br></br>
            <span>
              Requests by your web browser to our servers for web pages and
              other content on our website are recorded.
            </span>
            <span>28. Review of this privacy policy</span>
            <span>
              We shall continue to process this information until the contract
              between us ends or is terminated by either party under the terms
              of the contract.
            </span>
            <br></br>
            <br></br>
            <span>
              27.3. If you are in any way dissatisfied about how we process your
              personal information, you have a right to lodge a complaint.
              Please visit https://www.eugdpr.org/ to review information
              regarding the complaint process.
            </span>
            <br></br>
            <span></span>
            <span></span>
            <span>28. Retention period for personal data</span>
            <br></br>
            <br></br>
            <br></br>
            <span></span>
            <span>
              We use this information in aggregate to assess the popularity of
              the webpages on our website and how we perform in providing
              content to you.
            </span>
            <span>
              For example, some of the software our website uses may have been
              developed in the United States of America or in France or in
              another country.
            </span>
            <span>
              protecting your interests where we believe we have a duty to do so
            </span>
            <span></span>
            <br></br>
            <span></span>
            <span>
              responding to unsolicited communication from you to which we
              believe you would expect a response
            </span>
            <span></span>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <span>
              If your complaint reasonably requires us to contact some other
              person, we may decide to give to that other person some of the
              information contained in your complaint. We do this as
              infrequently as possible, but it is a matter for our sole
              discretion as to whether we do give information, and if we do,
              what that information is.
            </span>
            <br></br>
            <br></br>
            <span></span>
            <span></span>
            <br></br>
            <span></span>
            <span>7. Information we process with your consent</span>
            <span>
              Our website uses cookies. They are placed by software that
              operates on our servers, and by software operated by third parties
              whose services we use.
            </span>
            <span></span>
            <span></span>
            <br></br>
            <span>6.3. provide you with our services</span>
            <br></br>
            <br></br>
            <br></br>
            <span>14. Complaining</span>
            <span></span>
            <span>19. Information we obtain from third parties</span>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <span>
              Payment information is never taken by us or transferred to us
              either through our website or otherwise. Our employees and
              contractors never have access to it.
            </span>
            <br></br>
            <br></br>
            <span></span>
            <br></br>
            <span></span>
            <span></span>
            <span>
              9. Information we process because we have a legal obligation
            </span>
            <span></span>
            <span>
              In posting personal information, it is up to you to satisfy
              yourself about the privacy level of every person who might use it.
            </span>
            <br></br>
            <span>
              Cookies are small text files that are placed on your computer’s
              hard drive by your web browser when you visit any website. They
              allow information gathered on one web page to be stored until it
              is needed for use on another, allowing a website to provide you
              with a personalized experience and the website owner with
              statistics about how you use the website so that it can be
              improved.
            </span>
            <br></br>
            <br></br>
            <span>
              We may use a third party to provide us with re-marketing services
              from time to time. If so, then if you have consented to our use of
              cookies, you may see advertisements for our products and services
              on other websites.
            </span>
            <br></br>
            <span>11. Complaints regarding content on our website</span>
            <br></br>
            <span></span>
            <span>
              We record information such as your geographical location, your
              Internet service provider, and your IP address. We also record
              information about the software you are using to browse our
              website, such as the type of computer or device and the screen
              resolution.
            </span>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <span>
              We undertake to preserve the confidentiality of the information
              and of the terms of our relationship.
            </span>
            <span>26.3. to support a claim or defense in court.</span>
            <span></span>
            <br></br>
            <span></span>
            <br></br>
            <span>
              22.3. After receiving the request, we will tell you when we expect
              to provide you with the information, and whether we require any
              fee for providing it to you.
            </span>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <span></span>
            <span>
              We do store it, and we reserve a right to use it in the future in
              any way we decide.
            </span>
            <br></br>
            <span>15. Affiliate and business partner information</span>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <span>No such information is personally identifiable to you.</span>
            <span></span>
            <br></br>
            <span>
              Re-marketing involves placing a cookie on your computer when you
              browse our website in order to be able to serve to you an advert
              for our products or services when you visit some other website.
            </span>
            <span></span>
            <span>
              Third parties may advertise on our website. In doing so, those
              parties, their agents or other companies working for them may use
              technology that automatically collects information about you when
              their advertisement is displayed on our website.
            </span>
            <span></span>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <span>
              16.2. to record whether you have seen specific messages we display
              on our website
            </span>
            <span></span>
            <br></br>
            <br></br>
            <span>24. Verification of your information</span>
            <span></span>
            <br></br>
            <span>
              insuring against or obtaining professional advice that is required
              to manage risk related to our business
            </span>
            <br></br>
            <br></br>
            <span></span>
            <span></span>
            <span></span>
            <br></br>
            <span></span>
            <span></span>
            <br></br>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <span></span>
            <br></br>
            <span>
              When you create an account on our website, buy a product or
              service from us, or otherwise agree to our terms and conditions, a
              contract is formed between you and us.
            </span>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <span>
              For example, we may be required to give information to legal
              authorities if they so request or if they have the proper
              authorization such as a search warrant or court order.
            </span>
            <span></span>
            <span></span>
            <span></span>
            <span>13. Sending a message to our support team</span>
            <br></br>
            <span></span>
            <span></span>
            <br></br>
            <br></br>
            <span></span>
            <span></span>
            <br></br>
            <br></br>
            <span></span>
            <br></br>
            <span>
              Where we process your information on this basis, we do after
              having considered:
            </span>
            <br></br>
            <br></br>
            <span></span>
            <br></br>
            <span>This may include your personal information.</span>
            <span></span>
            <br></br>
            <br></br>
            <br></br>
            <span>
              26.2. to comply with other law, including for the period demanded
              by our tax authorities;
            </span>
            <br></br>
            <span>
              Your web browser should allow you to delete any you choose. It
              also should allow you to prevent or limit their use.
            </span>
            <br></br>
            <br></br>
            <span></span>
            <br></br>
            <span></span>
            <span></span>
            <span>
              26.1. to provide you with the services you have requested;
            </span>
            <br></br>
            <span>
              We use Secure Sockets Layer (SSL) certificates to verify our
              identity to your browser and to encrypt any data you give us.
            </span>
            <br></br>
            <br></br>
            <span></span>
            <span>10.1. posting a message our forum or comment form</span>
            <span>
              However, ultimately it is your choice as to whether you wish to
              use our website, products, and services.
            </span>
            <br></br>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <br></br>
            <span>
              Additionally, we may aggregate this information in a general way
              and use it to provide class information, for example, to monitor
              our performance with respect to a particular service we provide.
              If we use it for this purpose, you as an individual will not be
              personally identifiable.
            </span>
            <span>
              27.2. If a dispute is not settled then we hope you will agree to
              attempt to resolve it by engaging in good faith with us in a
              process of mediation or arbitration.
            </span>
            <span>6.1. verify your identity for security purposes</span>
            <span>16.3. to keep you signed in our site</span>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <br></br>
            <span></span>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <span></span>
            <span></span>
            <br></br>
            <span>
              If you do not consent to the use of cookies, the only recourse is
              to leave our website immediately.
            </span>
            <br></br>
            <br></br>
            <span></span>
            <span>
              If you complain about any of the content on our website, we shall
              investigate your complaint.
            </span>
            <span></span>
            <span>
              25.1. We do not sell products or provide services for purchase by
              children, nor do we market to children.
            </span>
            <br></br>
            <br></br>
            <span></span>
            <span></span>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <span>
              25.2. If you are under 18, you may use our website only with
              consent from a parent or guardian.
            </span>
            <span>
              27.1. If you are not happy with our privacy notice or privacy
              policy or if have any complaint then you should tell us by email.
              Our address is support@turbovirtual.com.
            </span>
            <span></span>
            <span></span>
            <span>
              16.1. to track how you use and interact with our website and your
              account
            </span>
            <br></br>
            <span>
              For example, we may process your data on this basis for the
              purposes of:
            </span>
            <br></br>
            <span>
              record-keeping for the proper and necessary administration of our
              official documents and customer records
            </span>
            <br></br>
            <span>
              whether processing (or not processing) might cause you harm
            </span>
            <span></span>
            <br></br>
            <span>protecting and asserting the legal rights of any party</span>
            <br></br>
            <br></br>
            <span></span>
            <span>
              If we feel it is justified or if we believe the law requires us to
              do so, we shall attempt to remove the content while we
              investigate. Keep in mind that blockchain data cannot be removed
              or deleted.
            </span>
            <br></br>
            <span>20. Third party advertising on our website</span>
            <span></span>
            <span>17. Personal identifiers from your browsing activity</span>
            <br></br>
            <br></br>
            <br></br>
            <span>
              10.4 using our software to create and mint a non-fungible token on
              our website and related public blockchain
            </span>
            <br></br>
            <span>26. Encryption of data sent between us</span>
            <br></br>
            <span>
              If we think your complaint is vexatious or without any basis, we
              shall not correspond with you about it.
            </span>
            <br></br>
            <span>
              whether you would expect us to process your data, and whether you
              would, in the round, consider it reasonable to do so
            </span>
            <br></br>
            <span></span>
            <br></br>
            <br></br>
            <span></span>
            <br></br>
            <span></span>
            <span></span>
            <br></br>
            <span></span>
            <span></span>
            <span></span>
          </span>
        </div>
        <footer className="privacy-policy-footer">
          <div className="privacy-policy-menu">
            <span className="privacy-policy-logo2">
              <span>
                VOLTAIC
                <span
                  dangerouslySetInnerHTML={{
                    __html: ' ',
                  }}
                />
              </span>
              <span className="privacy-policy-text305">Brands</span>
            </span>
            <div className="privacy-policy-links-container2">
              <div className="privacy-policy-container1">
                <svg viewBox="0 0 1024 1024" className="privacy-policy-icon11">
                  <path d="M854 342v-86l-342 214-342-214v86l342 212zM854 170q34 0 59 26t25 60v512q0 34-25 60t-59 26h-684q-34 0-59-26t-25-60v-512q0-34 25-60t59-26h684z"></path>
                </svg>
                <a
                  href="mailto:support@turbovirtual.com?subject=VOLTAIC  Support Inquiry"
                  className="privacy-policy-link06"
                >
                  Support
                </a>
              </div>
              <div className="privacy-policy-container2">
                <Link href="/privacy-policy">
                  <a className="privacy-policy-link07">Privacy Policy</a>
                </Link>
              </div>
              <div className="privacy-policy-container3">
                <Link href="/website-terms">
                  <a className="privacy-policy-link08">
                    Disclaimer &amp; Terms
                  </a>
                </Link>
              </div>
            </div>
            <div className="privacy-policy-follow-container1">
              <span className="privacy-policy-text306">
                Follow us on
                <span
                  dangerouslySetInnerHTML={{
                    __html: ' ',
                  }}
                />
              </span>
              <div className="privacy-policy-icons-container1">
                <a
                  href="https://instagram.com"
                  target="_blank"
                  rel="noreferrer noopener"
                  className="privacy-policy-link09"
                >
                  <svg
                    viewBox="0 0 877.7142857142857 1024"
                    className="privacy-policy-icon13"
                  >
                    <path d="M585.143 512c0-80.571-65.714-146.286-146.286-146.286s-146.286 65.714-146.286 146.286 65.714 146.286 146.286 146.286 146.286-65.714 146.286-146.286zM664 512c0 124.571-100.571 225.143-225.143 225.143s-225.143-100.571-225.143-225.143 100.571-225.143 225.143-225.143 225.143 100.571 225.143 225.143zM725.714 277.714c0 29.143-23.429 52.571-52.571 52.571s-52.571-23.429-52.571-52.571 23.429-52.571 52.571-52.571 52.571 23.429 52.571 52.571zM438.857 152c-64 0-201.143-5.143-258.857 17.714-20 8-34.857 17.714-50.286 33.143s-25.143 30.286-33.143 50.286c-22.857 57.714-17.714 194.857-17.714 258.857s-5.143 201.143 17.714 258.857c8 20 17.714 34.857 33.143 50.286s30.286 25.143 50.286 33.143c57.714 22.857 194.857 17.714 258.857 17.714s201.143 5.143 258.857-17.714c20-8 34.857-17.714 50.286-33.143s25.143-30.286 33.143-50.286c22.857-57.714 17.714-194.857 17.714-258.857s5.143-201.143-17.714-258.857c-8-20-17.714-34.857-33.143-50.286s-30.286-25.143-50.286-33.143c-57.714-22.857-194.857-17.714-258.857-17.714zM877.714 512c0 60.571 0.571 120.571-2.857 181.143-3.429 70.286-19.429 132.571-70.857 184s-113.714 67.429-184 70.857c-60.571 3.429-120.571 2.857-181.143 2.857s-120.571 0.571-181.143-2.857c-70.286-3.429-132.571-19.429-184-70.857s-67.429-113.714-70.857-184c-3.429-60.571-2.857-120.571-2.857-181.143s-0.571-120.571 2.857-181.143c3.429-70.286 19.429-132.571 70.857-184s113.714-67.429 184-70.857c60.571-3.429 120.571-2.857 181.143-2.857s120.571-0.571 181.143 2.857c70.286 3.429 132.571 19.429 184 70.857s67.429 113.714 70.857 184c3.429 60.571 2.857 120.571 2.857 181.143z"></path>
                  </svg>
                </a>
                <a
                  href="https://facebook.com"
                  target="_blank"
                  rel="noreferrer noopener"
                  className="privacy-policy-link10"
                >
                  <svg
                    viewBox="0 0 602.2582857142856 1024"
                    className="privacy-policy-icon15"
                  >
                    <path d="M548 6.857v150.857h-89.714c-70.286 0-83.429 33.714-83.429 82.286v108h167.429l-22.286 169.143h-145.143v433.714h-174.857v-433.714h-145.714v-169.143h145.714v-124.571c0-144.571 88.571-223.429 217.714-223.429 61.714 0 114.857 4.571 130.286 6.857z"></path>
                  </svg>
                </a>
                <a
                  href="https://twitter.com"
                  target="_blank"
                  rel="noreferrer noopener"
                  className="privacy-policy-link11"
                >
                  <svg
                    viewBox="0 0 950.8571428571428 1024"
                    className="privacy-policy-icon17"
                  >
                    <path d="M925.714 233.143c-25.143 36.571-56.571 69.143-92.571 95.429 0.571 8 0.571 16 0.571 24 0 244-185.714 525.143-525.143 525.143-104.571 0-201.714-30.286-283.429-82.857 14.857 1.714 29.143 2.286 44.571 2.286 86.286 0 165.714-29.143 229.143-78.857-81.143-1.714-149.143-54.857-172.571-128 11.429 1.714 22.857 2.857 34.857 2.857 16.571 0 33.143-2.286 48.571-6.286-84.571-17.143-148-91.429-148-181.143v-2.286c24.571 13.714 53.143 22.286 83.429 23.429-49.714-33.143-82.286-89.714-82.286-153.714 0-34.286 9.143-65.714 25.143-93.143 90.857 112 227.429 185.143 380.571 193.143-2.857-13.714-4.571-28-4.571-42.286 0-101.714 82.286-184.571 184.571-184.571 53.143 0 101.143 22.286 134.857 58.286 41.714-8 81.714-23.429 117.143-44.571-13.714 42.857-42.857 78.857-81.143 101.714 37.143-4 73.143-14.286 106.286-28.571z"></path>
                  </svg>
                </a>
              </div>
            </div>
          </div>
        </footer>
      </div>
      <style jsx>
        {`
          .privacy-policy-container {
            width: 100%;
            height: auto;
            display: flex;
            min-height: 100vh;
            align-items: center;
            flex-direction: column;
          }
          .privacy-policy-top-container {
            width: 100%;
            height: auto;
            display: flex;
            align-items: center;
            flex-direction: column;
            padding-bottom: var(--dl-space-space-unit);
            background-size: cover;
            background-image: linear-gradient(
                0deg,
                rgb(0, 0, 0) 14%,
                rgba(0, 0, 0, 0.24) 86%
              ),
              url('https://images.unsplash.com/photo-1632220894022-a83eacddae2d?ixid=Mnw5MTMyMXwwfDF8c2VhcmNofDMwfHxibG9ja2NoYWlufGVufDB8fHx8MTY1Nzg1NzE1OQ&ixlib=rb-1.2.1&w=1500');
            background-repeat: repeat, no-repeat;
            background-position: 0% 40%, center;
          }
          .privacy-policy-navbar {
            width: 100%;
            display: flex;
            max-width: 1000px;
            align-self: center;
            align-items: center;
            padding-top: var(--dl-space-space-doubleunit);
            padding-left: var(--dl-space-space-unit);
            padding-right: var(--dl-space-space-unit);
            flex-direction: row;
            padding-bottom: var(--dl-space-space-unit);
            justify-content: center;
          }
          .privacy-policy-link {
            color: var(--dl-color-gray-white);
            font-size: 2.25rem;
            font-family: Maven Pro;
            font-weight: 700;
            line-height: 1.25;
            text-transform: none;
            text-decoration: none;
          }
          .privacy-policy-text {
            font-family: Maven Pro;
            font-weight: 700;
            text-transform: none;
            text-decoration: none;
          }
          .privacy-policy-text001 {
            color: var(--dl-color-gray-700);
            font-style: normal;
            font-family: Maven Pro;
            font-weight: 400;
            text-transform: none;
            text-decoration: none;
          }
          .privacy-policy-text002 {
            color: var(--dl-color-gray-700);
            font-size: 14px;
            align-self: flex-start;
            margin-top: 12px;
          }
          .privacy-policy-right-side {
            display: none;
            align-items: center;
            flex-direction: row;
            justify-content: space-between;
          }
          .privacy-policy-links-container {
            display: flex;
            align-items: center;
            flex-direction: row;
            justify-content: space-between;
          }
          .privacy-policy-text003 {
            color: var(--dl-color-gray-white);
            margin-right: var(--dl-space-space-doubleunit);
          }
          .privacy-policy-text004 {
            color: var(--dl-color-gray-white);
            margin-right: var(--dl-space-space-doubleunit);
          }
          .privacy-policy-text005 {
            color: var(--dl-color-gray-white);
            margin-right: var(--dl-space-space-doubleunit);
          }
          .privacy-policy-text006 {
            color: var(--dl-color-gray-white);
            margin-right: var(--dl-space-space-doubleunit);
          }
          .privacy-policy-burger-menu {
            display: none;
            align-items: center;
            flex-direction: row;
            justify-content: space-between;
          }
          .privacy-policy-burger-menu {
            width: 24px;
            height: 24px;
          }
          .privacy-policy-mobile-menu {
            top: 0px;
            flex: 0 0 auto;
            left: 0px;
            width: 100%;
            height: 100%;
            display: none;
            padding: var(--dl-space-space-doubleunit);
            z-index: 100;
            position: absolute;
            flex-direction: column;
            justify-content: space-between;
            background-color: #fff;
          }
          .privacy-policy-nav {
            flex: 0 0 auto;
            display: flex;
            align-items: flex-start;
            flex-direction: column;
          }
          .privacy-policy-top {
            flex: 0 0 auto;
            width: 100%;
            display: flex;
            align-items: center;
            margin-bottom: var(--dl-space-space-doubleunit);
            flex-direction: row;
            justify-content: space-between;
          }
          .privacy-policy-close-menu {
            flex: 0 0 auto;
            display: flex;
            align-items: flex-start;
            flex-direction: column;
          }
          .privacy-policy-icon03 {
            width: 24px;
            height: 24px;
          }
          .privacy-policy-right-side1 {
            width: 100%;
            display: flex;
            align-items: flex-start;
            flex-direction: column;
            justify-content: space-between;
          }
          .privacy-policy-links-container1 {
            display: flex;
            align-items: flex-start;
            margin-bottom: 16px;
            flex-direction: column;
            justify-content: space-between;
          }
          .privacy-policy-text007 {
            margin-bottom: 8px;
          }
          .privacy-policy-text008 {
            margin-bottom: 8px;
          }
          .privacy-policy-text009 {
            margin-bottom: 8px;
          }
          .privacy-policy-link02 {
            display: contents;
          }
          .privacy-policy-follow-container {
            display: flex;
            align-items: flex-start;
            flex-direction: column;
            justify-content: space-between;
          }
          .privacy-policy-text011 {
            padding-bottom: var(--dl-space-space-halfunit);
          }
          .privacy-policy-icons-container {
            width: 100px;
            display: flex;
            align-items: flex-start;
            flex-direction: row;
            justify-content: space-between;
          }
          .privacy-policy-link03 {
            display: contents;
          }
          .privacy-policy-icon05 {
            width: 24px;
            height: 24px;
          }
          .privacy-policy-link04 {
            display: contents;
          }
          .privacy-policy-icon07 {
            width: 24px;
            height: 24px;
          }
          .privacy-policy-link05 {
            display: contents;
          }
          .privacy-policy-icon09 {
            width: 24px;
            height: 24px;
          }
          .privacy-policy-hero {
            flex: 1;
            width: 100%;
            display: flex;
            max-width: 1000px;
            align-items: center;
            padding-left: var(--dl-space-space-doubleunit);
            padding-right: var(--dl-space-space-doubleunit);
            flex-direction: column;
            justify-content: center;
          }
          .privacy-policy-content-container {
            display: flex;
            align-items: center;
            padding-top: var(--dl-space-space-tripleunit);
            flex-direction: column;
            padding-bottom: var(--dl-space-space-doubleunit);
            justify-content: flex-start;
          }
          .privacy-policy-text012 {
            color: var(--dl-color-gray-white);
            font-size: 2.5rem;
            text-align: center;
            font-family: Maven Pro;
            font-weight: 700;
            line-height: 1.25;
            margin-bottom: var(--dl-space-space-halfunit);
            text-transform: none;
            text-decoration: none;
          }
          .privacy-policy-main {
            width: 100%;
            display: flex;
            align-items: center;
            padding-top: var(--dl-space-space-tripleunit);
            padding-left: var(--dl-space-space-doubleunit);
            padding-right: var(--dl-space-space-doubleunit);
            flex-direction: column;
            padding-bottom: var(--dl-space-space-quadruple);
            justify-content: flex-start;
          }
          .privacy-policy-text014 {
            max-width: 1280px;
          }
          .privacy-policy-footer {
            color: var(--dl-color-gray-white);
            width: 100%;
            display: flex;
            padding: var(--dl-space-space-tripleunit);
            align-items: center;
            border-color: var(--dl-color-gray-900);
            border-width: 1px;
            flex-direction: row;
            justify-content: center;
            background-color: var(--dl-color-gray-black);
            border-left-width: 0px;
            border-right-width: 0px;
            border-bottom-width: 0px;
          }
          .privacy-policy-menu {
            width: 100%;
            display: flex;
            max-width: 1000px;
            align-items: center;
            flex-direction: row;
            justify-content: space-between;
          }
          .privacy-policy-logo2 {
            font-size: 1.25em;
          }
          .privacy-policy-text305 {
            color: var(--dl-color-gray-700);
          }
          .privacy-policy-links-container2 {
            display: flex;
            align-items: center;
            flex-direction: row;
            justify-content: space-between;
          }
          .privacy-policy-container1 {
            display: flex;
            margin-left: 0px;
            flex-direction: row;
          }
          .privacy-policy-icon11 {
            fill: var(--dl-color-gray-white);
            width: 18px;
            height: 18px;
            margin-right: var(--dl-space-space-halfunit);
          }
          .privacy-policy-link06 {
            font-size: 14px;
            text-decoration: none;
          }
          .privacy-policy-container2 {
            display: flex;
            align-items: center;
            margin-left: 100px;
            flex-direction: row;
          }
          .privacy-policy-link07 {
            font-size: 14px;
            text-decoration: none;
          }
          .privacy-policy-container3 {
            display: flex;
            align-items: center;
            margin-left: 100px;
            flex-direction: row;
          }
          .privacy-policy-link08 {
            font-size: 14px;
            text-decoration: none;
          }
          .privacy-policy-follow-container1 {
            display: none;
            align-items: center;
            margin-left: 50px;
            flex-direction: column;
            justify-content: space-between;
          }
          .privacy-policy-text306 {
            padding-bottom: var(--dl-space-space-halfunit);
          }
          .privacy-policy-icons-container1 {
            width: 100px;
            display: flex;
            align-items: flex-start;
            flex-direction: row;
            justify-content: space-between;
          }
          .privacy-policy-link09 {
            display: contents;
          }
          .privacy-policy-icon13 {
            fill: var(--dl-color-gray-white);
            width: 24px;
            height: 24px;
          }
          .privacy-policy-link10 {
            display: contents;
          }
          .privacy-policy-icon15 {
            fill: var(--dl-color-gray-white);
            width: 24px;
            height: 24px;
          }
          .privacy-policy-link11 {
            display: contents;
          }
          .privacy-policy-icon17 {
            fill: var(--dl-color-gray-white);
            width: 24px;
            height: 24px;
          }
          @media (max-width: 991px) {
            .privacy-policy-top-container {
              background-image: linear-gradient(
                  0deg,
                  rgb(0, 0, 0) 0%,
                  rgba(0, 0, 0, 0.5) 86%
                ),
                url('https://images.unsplash.com/photo-1632220894022-a83eacddae2d?ixid=Mnw5MTMyMXwwfDF8c2VhcmNofDMwfHxibG9ja2NoYWlufGVufDB8fHx8MTY1Nzg1NzE1OQ&ixlib=rb-1.2.1&w=1500');
              background-repeat: repeat, no-repeat;
              background-position: 0% 40%, center;
            }
            .privacy-policy-hero {
              padding-left: var(--dl-space-space-tripleunit);
              padding-right: var(--dl-space-space-tripleunit);
            }
            .privacy-policy-container1 {
              margin-left: 50px;
            }
            .privacy-policy-container2 {
              margin-left: 50px;
            }
            .privacy-policy-container3 {
              margin-left: 50px;
            }
          }
          @media (max-width: 767px) {
            .privacy-policy-top-container {
              background-image: linear-gradient(
                  0deg,
                  rgb(0, 0, 0) 0%,
                  rgba(0, 0, 0, 0.6) 86%
                ),
                url('https://images.unsplash.com/photo-1632220894022-a83eacddae2d?ixid=Mnw5MTMyMXwwfDF8c2VhcmNofDMwfHxibG9ja2NoYWlufGVufDB8fHx8MTY1Nzg1NzE1OQ&ixlib=rb-1.2.1&w=1500');
              background-repeat: repeat, no-repeat;
              background-position: 0% 40%, center;
            }
            .privacy-policy-right-side {
              display: none;
            }
            .privacy-policy-burger-menu {
              display: none;
            }
            .privacy-policy-hero {
              justify-content: center;
            }
            .privacy-policy-content-container {
              align-items: center;
            }
            .privacy-policy-main {
              padding-left: var(--dl-space-space-unit);
              padding-right: var(--dl-space-space-unit);
            }
            .privacy-policy-menu {
              flex-direction: column;
            }
            .privacy-policy-links-container2 {
              margin-top: var(--dl-space-space-tripleunit);
              margin-bottom: var(--dl-space-space-tripleunit);
              justify-content: space-between;
            }
            .privacy-policy-container1 {
              margin-left: 40px;
            }
            .privacy-policy-container2 {
              margin-left: 40px;
            }
            .privacy-policy-container3 {
              margin-left: 40px;
            }
          }
          @media (max-width: 479px) {
            .privacy-policy-top-container {
              height: auto;
              padding-top: var(--dl-space-space-doubleunit);
              padding-bottom: var(--dl-space-space-doubleunit);
              background-image: linear-gradient(
                  0deg,
                  rgb(0, 0, 0) 2%,
                  rgba(0, 0, 0, 0.5) 100%
                ),
                url('https://images.unsplash.com/photo-1632220894022-a83eacddae2d?ixid=Mnw5MTMyMXwwfDF8c2VhcmNofDMwfHxibG9ja2NoYWlufGVufDB8fHx8MTY1Nzg1NzE1OQ&ixlib=rb-1.2.1&w=1500');
            }
            .privacy-policy-burger-menu {
              display: none;
            }
            .privacy-policy-text012 {
              font-size: 1.75rem;
            }
            .privacy-policy-links-container2 {
              margin-top: var(--dl-space-space-doubleunit);
              align-items: flex-start;
              flex-direction: column;
              justify-content: flex-start;
            }
            .privacy-policy-container1 {
              align-self: center;
              margin-left: 0px;
              margin-bottom: var(--dl-space-space-halfunit);
            }
            .privacy-policy-link06 {
              margin-bottom: var(--dl-space-space-halfunit);
            }
            .privacy-policy-container2 {
              height: 100%;
              align-self: center;
              align-items: center;
              margin-left: 0px;
              margin-bottom: var(--dl-space-space-halfunit);
            }
            .privacy-policy-link07 {
              margin-bottom: var(--dl-space-space-halfunit);
            }
            .privacy-policy-container3 {
              height: 100%;
              align-self: center;
              align-items: center;
              margin-left: 0px;
            }
            .privacy-policy-link08 {
              margin-bottom: var(--dl-space-space-halfunit);
            }
            .privacy-policy-follow-container1 {
              margin-left: 0px;
            }
          }
        `}
      </style>
    </>
  )
}

export default PrivacyPolicy
